# Gliederung ERT:

Hauptfrage:
- Welche Lernmaterialien brauchen die Studierenden/bringen den Studierenden Lernerfolg/was ist den Studierenden wichtig an den Lernmaterialien.

Folgefragen:
- gibt es Zusammenhänge?

Titelblatt
 Vakatseite
 I Inhaltsverzeichnis
 1 VakatseiteI

1. Einleitung (= Motivation, Forschungsfrage(n), ca. 1/2 Seite)

Lernerfolg -> schwierig
materialien dafür -> wichtig
abba welche und wie?? -> dafür unsere Studie, als Fragebogen! (warum fragebogen)
-- konkrete Fragestellungen
dafür haben wir einen online fragebogen erstellt, menschen befragt/ausgewertet, hier beschrieben
was ist in diesem bericht zu lesen... bla bla

2. Erhebungsinstrument (= Überblick Fragebogen, ca. 1 Seite
(zu zweit) oder 1.5 Seiten (zu dritt))

konkret warum fragebogen
Konkret warum genau diese fragen
wichtig!! erhebungsmethoden -> QR codes auf der UNI strasse
wichtiger! auf welcher weise ist der datset dadurch verzerrt?
(in unserem fall-> wir haben durch den erhebungsort/zeit meistens junge statistiker)
hier datenschutz erwähnen! (yay)
das platform womit de rfragebogen erstellt wurde

3. Stichprobe und Datensatz (= Erhebung, Stichprobe,
erhobene Variablen inkl. Skalenniveaus, ca. 1 Seite)

in welcher Form wir den Datensatz bekommen haben -> datensatz womit wir im eneffekt arbeiten

dies bezieht sich auf:
- umnennen von variablen
- datenbereinigung
- beschreiben von na werten
- umskalieren auf 1-5

wichtig: wie viele Variablen (was für, ordinal, kategorial etc), wie viele na werte

4. Ergebnisse (Grafiken, Tabellen, begleitender Text, ca. 4
Seiten (zu zweit), ca. 5 Seiten (zu dritt))

bevor: kurze beschreibung den von verwendetetn Software (R oder python) + bibliotheken

- quasi demografie
hier die verteilung von Semster, Studiengängen etc (2 abbildungen + beschreibung mit wörtert was man alle ssieht)

- Hauptfragestellung
Abbildung von wegen: das finden Studierende hilfreich + beschreibung

- explorative: ka was auch immer wir finden


5. Diskussion (= Einordnung/Interpretation der Ergebnisse in
Relation zu Forschungsfragen, Limitationen, ca. 1 Seite (zu
zweit) oder 1.5 Seiten (zu dritt))

- ergebnisse sind so und so
- die sind verzerrt durch so und so
- wir interpretieren dies so und so

6. Reflexion der fragebogengestützten Erhebung (= Was
müsste bei Fragebogen bzw. Erhebung rückblickend anders
gemacht werden? ca. 1 Seite)

- wir haben 1nen beschissenen job gemacht weil...
verzerrrungen/komische reinigung, nicht alles darstellbar usw
 
  1 VakatseiteI bis zu 7 (alleine) bzw. 9 (zu zweit), bzw. 11 (zu dritt) Seiten
eigentlicher BerichtI Literaturverzeichnis (nur wenn nötig, im Zweifelsfall im
APA-Stil zitieren)I 

Appendix: Verwendeter FragebogenI Appendix: Erklärung zu Anteilen an der Textproduktion